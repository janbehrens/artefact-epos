...
class PluginArtefactEpos extends PluginArtefact {
    ...
    public static function get_artefact_types() {
        return array('learnedlanguage', 'checklist', 'customgoal', 'diary', 'diaryentry');
    }
...
/**
 * A Diary artefact is a collection of DiaryEntry artefacts.
 */
class ArtefactTypeDiary extends ArtefactType {
    ...
    public function describe_size() {
        return $this->count_children() . ' ' . get_string('posts', 'artefact.epos');
    }
    ...
    public function render_self($options) {
        ...
        $posts = ArtefactTypeDiaryEntry::get_posts($this->id, $limit, $offset, $options);
        ...
        ArtefactTypeDiaryEntry::render_posts($posts, $template, $options, $pagination);
    ...
    public static function get_blog_list($limit, $offset) {
        ...
        ($result = get_records_sql_array("
         ...
         FROM {artefact} b LEFT JOIN {artefact} p ON (p.parent = b.id AND p.artefacttype = 'diaryentry')
         WHERE b.owner = ? AND b.artefacttype = 'diary'
        ...
        foreach ($result as &$r) {
            if (!$r->locked) {
                $r->deleteform = ArtefactTypeDiary::delete_form($r->id);
            }
        }

        $count = (int)get_field('artefact', 'COUNT(*)', 'owner', $USER->get('id'), 'artefacttype', 'diary');
    ...
    public static function build_blog_list_html(&$blogs) {
        ...
        $blogs->tablerows = $smarty->fetch('artefact:epos:diarylist.tpl');
        $pagination = build_pagination(array(
            'id' => 'bloglist_pagination',
            'class' => 'center',
            'url' => get_config('wwwroot') . 'artefact/epos/diary.php',
            ...
            'resultcounttextsingular' => get_string('blog', 'artefact.epos'),
            'resultcounttextplural' => get_string('blogs', 'artefact.epos'),
        ));
    ...
    public static function new_blog(User $user, array $values) {
        $artefact = new ArtefactTypeDiary();
    ...
    public static function edit_blog(User $user, array $values) {
        ...
        $artefact = new ArtefactTypeDiary($values['id']);
    ...
    public static function get_links($id) {
        ...
        return array(
            ...
            get_string('blogsettings', 'artefact.epos') => $wwwroot . 'artefact/blog/settings/?id=' . $id,
        );
    }
        ...
        return pieform(array(
            ...
            'elements' => array(
                ...
                'submit' => array(
                    ...
                    'elementtitle' => get_string('delete', 'artefact.epos'),
                    'confirm' => get_string('deleteblog?', 'artefact.epos'),
                ),
            ),
        ));
    }
 ...
 */
class ArtefactTypeDiaryEntry extends ArtefactType {
    ...
    public function __construct($id = 0, $data = null) {
        ...
        if ($this->id) {
            if ($bpdata = get_record('artefact_blog_blogpost', 'blogpost', $this->id)) {
                ...
                throw new ArtefactNotFoundException(get_string('blogpostdoesnotexist', 'artefact.epos'));
            }
        }
        else {
    ...
    public function check_permission() {
        global $USER;
        if ($USER->get('id') != $this->owner) {
            throw new AccessDeniedException(get_string('youarenottheownerofthisblogpost', 'artefact.epos'));
        }
    }
  
    public function describe_size() {
        return $this->count_attachments() . ' ' . get_string('attachments', 'artefact.epos');
    }
    ...
    public function render_self($options) {
        ...
        $smarty->assign('postedbyon', get_string('postedbyon', 'artefact.epos',
    ...
    public static function get_posts($id, $limit, $offset, $viewoptions=null) {
        ...
        $from = "
            ...
            WHERE a.artefacttype = 'diaryentry' AND a.parent = ?";
        ...
        foreach ($data as &$post) {
            ...
            if (is_null($viewoptions)) {
                ...
                if (!$post->published) {
                    $post->publish = ArtefactTypeDiaryEntry::publish_form($post->id);
                }
                $post->delete = ArtefactTypeDiaryEntry::delete_form($post->id);
            }
            else {
    ...
    public function render_posts(&$posts, $template, $options, $pagination) {
        ...
        if ($posts['limit'] && $pagination) {
            $pagination = build_pagination(array(
                ...
                'offset' => $posts['offset'],
                'numbersincludefirstlast' => false,
                'resultcounttextsingular' => get_string('post', 'artefact.epos'),
                'resultcounttextplural' => get_string('posts', 'artefact.epos'),
            ));
    ...
    public static function new_post(User $user, array $values) {
        $artefact = new ArtefactTypeDiaryEntry();
        $artefact->set('title', $values['title']);
        $artefact->set('description', $values['description']);
    ...
    public static function edit_post(User $user, array $values) {
        $artefact = new ArtefactTypeDiaryEntry($values['id']);
        if ($user->get('id') != $artefact->get('owner')) {
            return false;
        }
        ...
        return pieform(array(
            ...
            'elements' => array(
                ...
                'submit' => array(
                    'type' => 'submit',
                    'class' => 'publish',
                    'value' => get_string('publish', 'artefact.epos'),
                ),
            ),
        ));
    }
        ...
        return pieform(array(
            ...
            'elements' => array(
                ...
                'submit' => array(
                    ...
                    'src' => $THEME->get_url('images/icon_close.gif'),
                    'elementtitle' => get_string('delete', 'artefact.epos'),
                    'confirm' => get_string('deleteblogpost?', 'artefact.epos'),
                ),
            ),
        ));
    }
    ...
    public function default_parent_for_copy(&$view, &$template, $artefactstoignore) {
        ...
        $blogname = get_string('viewposts', 'artefact.epos', $view->get('id'));
        $data = (object) array(
            'title'       => $blogname,
            'description' => get_string('postscopiedfromview', 'artefact.epos', $template->get('title')),
            'owner'       => $view->get('owner'),
            'group'       => $view->get('group'),
            'institution' => $view->get('institution'),
        );
        $blog = new ArtefactTypeDiary(0, $data);
        $blog->commit();
