...
define('TITLE', get_string('viewblog','artefact.epos'));
safe_require('artefact', 'epos');
require_once(get_config('libroot') . 'pieforms/pieform.php');

if ($publish = param_integer('publish', null)) {
    ArtefactTypeDiaryEntry::publish_form($publish);
}
if ($delete = param_integer('delete', null)) {
    ArtefactTypeDiaryEntry::delete_form($delete);
}
...
if (is_null($id)) {
    ...
    $id = $records[0]->id;
    $blog = new ArtefactTypeDiary($id, $records[0]);
}
else {
...
$posts = ArtefactTypeDiaryEntry::get_posts($id, $limit, $offset);
$template = 'artefact:blog:posts.tpl';
$pagination = array(
    'baseurl'    => get_config('wwwroot') . 'artefact/epos/diaryview.php?id=' . $id,
    'id'         => 'blogpost_pagination',
    'jsonscript' => 'artefact/blog/view/index.json.php',
    'datatable'  => 'postlist',
);
ArtefactTypeDiaryEntry::render_posts($posts, $template, array(), $pagination);

$strpublished = json_encode(get_string('published', 'artefact.epos'));
$js = <<<EOF
...
$smarty->display('artefact:epos:diaryview.tpl');
exit;
...
function publish_submit(Pieform $form, $values) {
    ...
    $form->reply(PIEFORM_OK, array(
        'message' => get_string('blogpostpublished', 'artefact.epos'),
        'goto' => get_config('wwwroot') . 'artefact/epos/diaryview.php?id=' . $blogpost->get('parent'),
        'id' => $values['publish'],
    ));
}
...
function delete_submit(Pieform $form, $values) {
    ...
    $blogpost->delete();
    $form->reply(PIEFORM_OK, array(
        'message' => get_string('blogpostdeleted', 'artefact.epos'),
        'goto' => get_config('wwwroot') . 'artefact/epos/diaryview.php?id=' . $blogpost->get('parent'),
        'id' => $values['delete'],
    ));
}
